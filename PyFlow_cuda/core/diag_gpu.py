...
def int_ode_nloc(l,y,method='einsum'):
        ...
        sol2 = contract(eta_int,H,method=method) + contract(eta0,Hint,method=method)
...
def int_liom(l,y,method='einsum'):
        ...
        eta0 = contract(H0,V0,method=method)
        # print(l)
        # print(l,torch.linalg.norm(eta0))
        # print(eta0)
        eta_int = contract(Hint0,V0,method=method) + contract(H0,Vint,method=method)
        ...
        return -1*torch.cat([torch.cat([soln,sol2]),torch.cat([nsoln,nsol2])],axis=0)
...
def event(t,y):
    ...
    V2 = V2[V2!=0.]
    # print(torch.max(torch.abs(V)))
    #print('lenV',len(V2),len(V2)//20)
    if t < 0.5 or sum(i>cutoff for i in V2)>n**2//20:
        return torch.tensor(1,device=device)
    elif len(V2)>1 and max(torch.abs(V2))>0.05:
        return torch.tensor(1,device=device)
    else:
        #print(t,torch.max(torch.abs(V)))
        return torch.tensor(0,device=device)

def flow_static_int_torch(n,J,H0,V0,Hint,Vint,dl_list,qmax,cutoff,method='einsum'):
    ...
    # FE Diag (CPU)
        ...
        while k < len(dl_list)-1 and J0 > cutoff:
            ...
            sol_list[k+1] = sol[-1]
            H0_diag = sol[-1,0,0]
            V = H0_diag - torch.diag(torch.diag(H0_diag))
            J0 = torch.max(torch.abs(V))
            #print(k,J0)
            k += 1
        print(k,J0)
        dl_list = dl_list[0:k]
        sol_list = sol_list[0:k]

        # liom2 = torch.Tensor.numpy(torch.Tensor.cpu(sol_list[::,0,0]))
        # for i in range(n):
        #     plt.plot(dl_list,liom2[::,i,i],'o-')
            # for j in range(n):
            #     if i != j:
            #         plt.plot(dl_list,liom2[::,i,j],'x--')
        ...
        while k < len(dl_list):
            ...
            var = torch.cat([torch.cat([H,Hint2]),torch.cat([l2,l4])],axis=0)
            liom_flow[k] = liom[-1]
            # print(k,dl_list[k:k+2])
            k += 1
 
        # dl_list = torch.flip(dl_list,[0])
        # for i in range(len(dl_list)):
        #     dl_list[i] += -1*torch.min(dl_list)
        # print(dl_list)
        liom_flow = torch.flip(liom_flow,[0])

        # liom2 = torch.Tensor.numpy(torch.Tensor.cpu(liom_flow[::,2*n,0]))
        # for i in range(n):
        #     # plt.plot(dl_list,liom2[::,i,i],'x--')
        #     for j in range(n):
        #         # if i != j:
        #             plt.plot(dl_list,liom2[::,i,j],'o-')
        # plt.show()
        # plt.close()

        print('l4max',torch.max(torch.abs(liom_flow)))

 
        # liom4 = torch.Tensor.numpy(torch.Tensor.cpu(liom_flow[::,3*n:]))
        # for i in range(n):
        #     for j in range(n):
        #         # for k in range(n):
        #         #     for q in range(n):
        #             plt.plot(dl_list,liom4[::,i,i,j,j],'o-')
        #             plt.plot(dl_list,liom4[::,i,j,j,i],'o-')
        # plt.show()
        # plt.close()
        ...
        l2 = torch.Tensor.numpy(torch.Tensor.cpu(liom[-1,2*n,0]))
        l4 = torch.Tensor.numpy(torch.Tensor.cpu(liom[-1,3*n:]))
        
        liom_fwd=np.concatenate((n2.reshape(n**2),nint.reshape(n**4)))
        liom_bck=np.concatenate((l2.reshape(n**2),l4.reshape(n**4)))

        # print(l2)
        #plt.plot(np.log10(np.diag(np.abs(l2))))
        # plt.show()
        # plt.close()
        ...
        return([H0_diag,Hint,lbits,np.diag(n2),liom_fwd,liom_bck])
